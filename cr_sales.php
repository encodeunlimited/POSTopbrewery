         ...
         <form action="" class="h-100" id="transaction-form">
             <div class="w-100 h-100 mx-0 row row-cols-2 bg-dark">
                 <div class="col-8 h-100 pb-2 d-flex flex-column">
                     <div>
                         <h3 class="text-light">Please select Credit Tyres below</h3>
                     </div>
                                         ...
                                         <?php
                                            ...
                                            ?>
                                             <tr class="item <?php echo $qty < 50 ? "bg-danger bg-opacity-25" : '' ?>" data-id="<?php echo $row['product_id'] ?>">
                                                 ...
                                                 <td class="td py-0 px-1 text-start price"><?php echo number_format($row['creditprice'], 2) ?></td>
                                                 ...
                                                 <td class="td py-0 px-1 text-end profit"><?php echo ($row['creditprice'] - $row['buyprice']) ?></td>
                                             </tr>
                                         <?php endwhile; ?>
                                     </tbody>
                                 </table>
                         ...
                         <div class="w-100 flex-grow-1">
                             <div class="h-100 d-flex w-100 flex-column">
                                 ...
                                 <div class="pt-2">
                                     ...
                                     <div class="w-100 mx-0 d-flex pb-1">
                                         <div class="col-4 pe-2 fs-6 fw-bolder text-light">Client</div>
                                         <div class="flex-grow-1 bg-light fs-6 fw-bolder text-end px-2" id="total">
                                             <select name="client_id" id="client_id" class="form-select form-select-sm rounded-0" required>
                                                 <option <?php echo (!isset($client_id)) ? 'selected' : '' ?> disabled>Please Select Here</option>
                                                 <?php
                                                    $sup_qry = $conn->query("SELECT * FROM cclient_list where `status` = 1  order by `name` asc");
                                                    while ($row = $sup_qry->fetchArray()) :
                                                    ?>
                                                     <option value="<?php echo $row['client_id'] ?>" <?php echo (isset($client_id) && $client_id == $row['client_id']) ? 'selected' : '' ?>><?php echo $row['name'] ?></option>
                                                 <?php endwhile; ?>
                                             </select>
                                         </div>
                                     </div>
             ...
             <input type="hidden" name="dnic" value="">
 ...
 <script>
     $(function() {
         ...
         $('#transaction-save-btn').click(function() {
             ...
             uni_modal("Payment", "ctender_amount.php?amount=" + $('[name="total"]').val() + "&tprofit=" + $('[name="t_profit"]').val())
         ...
         $('#transaction-form').submit(function(e) {
             ...
             $.ajax({
                 url: './Actions.php?a=save_ctransaction',
                 ...
                 success: function(resp) {
                     if (resp.status == 'success') {
                         setTimeout(() => {
                             uni_modal("RECEIPT", "cview_receipt.php?id=" + resp.transaction_id)
                         }, 1000);
                     } else {
                         _el.addClass('alert alert-danger')
                     }
