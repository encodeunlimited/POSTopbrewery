...
if (isset($_GET['id'])) {
    $qry = $conn->query("SELECT * FROM `ctransaction_list` where transaction_id = '{$_GET['id']}'");
            ...
            <thead>
                ...
                <tr>
                    <td style="font-size: small; text-align: left;">
                        ...
                        Receipt No : <?php echo $receipt_no ?><br>
                        <?php
                        $sql = "SELECT p.*,c.name as cname FROM `ctransaction_list` p inner join `cclient_list` c on p.client_id = c.client_id where p.transaction_id = '{$_GET['id']}'";
                        $qry = $conn->query($sql);
                        $i = 1;
                        while ($row = $qry->fetchArray()) :
                        ?>
                            Customer : <?php echo $row['cname'] ?><br>
                    ...
                    <td style="font-size: small; text-align: left;">
                    Vehical No : <?php echo $vehical_no?><br>
                    Driver NIC : <?php echo $dnic ?><br>
                    Driver Name : <?php echo $dname ?><br>
                    </td>
                </tr>
            </thead>
            <tbody></tbody>
