        ...
        <div id="outprint">
            <table class="table table-hover table-striped table-bordered" id="print">
                ...
                <tbody>
                    ...
                    $sql = "SELECT t.*, u.fullname FROM refund_transaction_list t INNER JOIN user_list u ON t.user_id = u.user_id WHERE strftime('%Y-%m-%d %H:%M:%S', t.date_added) BETWEEN '{$dfrom}' AND '{$dto}' {$user_where} {$pay_query} ORDER BY strftime('%s', t.date_added) DESC;";
                    $qry = $conn->query($sql);
                    ...
                    while ($row = $qry->fetchArray()) :
                        $items = $conn->query("SELECT sum(quantity) as `count` FROM `refund_transaction_items` where transaction_id = '{$row['transaction_id']}'  ")->fetchArray()['count'];
                    ...
                    <?php if (!$qry->fetchArray()) : ?>
                        <th colspan="6">
                            <center>No Refund Transaction listed in selected date.</center>
                        </th>
                    <?php endif; ?>
                ...
                <tr class="bg-dark bg-opacity-25 text-bold">
                    ...
                    <td class="px-1 py-0 text-end"><?php
                                                    if ($_SESSION['type'] == 1) {
                                                        $total = $conn->query("SELECT sum(total) as `total` FROM `refund_transaction_list` WHERE strftime('%Y-%m-%d %H:%M:%S', date_added) BETWEEN '{$dfrom}' AND '{$dto}' {$pay_query1}")->fetchArray()['total'];
                                                    } else {
                                                        $total = $conn->query("SELECT sum(total) as `total` FROM `refund_transaction_list` WHERE strftime('%Y-%m-%d %H:%M:%S', date_added) BETWEEN '{$dfrom}' AND '{$dto}' {$user_c} {$pay_query1}")->fetchArray()['total'];
                                                    }
                    ...
                    <td class="px-1 py-0 text-end"><?php
                                                    if ($_SESSION['type'] == 1) {
                                                        $t_profit = $conn->query("SELECT sum(t_profit) as `total_profit` FROM `refund_transaction_list` WHERE strftime('%Y-%m-%d %H:%M:%S', date_added) BETWEEN '{$dfrom}' AND '{$dto}' {$pay_query1}")->fetchArray()['total_profit'];
                                                    } else {
                                                        $t_profit = $conn->query("SELECT sum(t_profit) as `total_profit` FROM `refund_transaction_list` WHERE strftime('%Y-%m-%d %H:%M:%S', date_added) BETWEEN '{$dfrom}' AND '{$dto}' {$user_c} {$pay_query1}")->fetchArray()['total_profit'];
                                                    }
                    ...
                    <!-- <td class="px-1 py-0 text-end"><?php
                                                        if ($_SESSION['type'] == 1) {
                                                            $arrears = $conn->query("SELECT sum(arrears) as `total_arrears` FROM `refund_transaction_list` WHERE strftime('%Y-%m-%d %H:%M:%S', date_added) BETWEEN '{$dfrom}' AND '{$dto}' {$pay_query1}")->fetchArray()['total_arrears'];
                                                        } else {
                                                            $arrears = $conn->query("SELECT sum(arrears) as `total_arrears` FROM `refund_transaction_list` WHERE strftime('%Y-%m-%d %H:%M:%S', date_added) BETWEEN '{$dfrom}' AND '{$dto}' {$user_c} {$pay_query1}")->fetchArray()['total_arrears'];
                                                        }
...
<script>
    $(function() {
        ...
        $('.view_data').click(function() {
            uni_modal('Receipt', "view_refund_receipt.php?view_only=true&id=" + $(this).attr('data-id'), '')
        })
        $('#filter').click(function() {
            location.href = "./?page=refund_report&date_from=" + $('#date_from').val() + "&date_to=" + $('#date_to').val() + "&payment_method=" + $('#payment_method').val();
        })
