    ...
    <div class="card-body">
        ...
        <table class="table table-hover table-striped table-bordered">
            ...
            <tbody>
                ...
                $sql = "SELECT t.*,u.fullname,c.name FROM  `ctransaction_list` t inner join user_list u on t.user_id = u.user_id INNER JOIN cclient_list c ON t.client_id = c.client_id where date(t.date_added) between '{$dfrom}' and '{$dto}' {$user_where}  order by strftime('%s',date_added) desc";
                $qry = $conn->query($sql);
                ...
                <tr>
                    ...
                    <td class="py-0 px-1 text-end"><?php echo number_format($row['tendered_amount'],2) ?></td>
                ...
                if($_SESSION['type']==1){$total = $conn->query("SELECT sum(tendered_amount) as `total` FROM `ctransaction_list` where date(date_added) between '{$dfrom}' and '{$dto}'")->fetchArray()['total'];}
                else{$total = $conn->query("SELECT sum(tendered_amount) as `total` FROM `ctransaction_list` where date(date_added) between '{$dfrom}' and '{$dto}' {$user_c}")->fetchArray()['total'];}
                echo number_format($total,2) ;
                    ?>
                </td>
                <td class="px-1 py-0 text-end"><?php 
                if($_SESSION['type']==1){$t_profit = $conn->query("SELECT sum(t_profit) as `total_profit` FROM `ctransaction_list` where date(date_added) between '{$dfrom}' and '{$dto}'")->fetchArray()['total_profit'];}
                else{$t_profit = $conn->query("SELECT sum(t_profit) as `total_profit` FROM `ctransaction_list` where date(date_added) between '{$dfrom}' and '{$dto}' {$user_c}")->fetchArray()['total_profit'];}
                echo number_format($t_profit,2) ;
                    ?>
                </td>
                <td class="px-1 py-0 text-end"><?php 
                if($_SESSION['type']==1){$arrears = $conn->query("SELECT sum(arrears) as `total_arrears` FROM `ctransaction_list` where date(date_added) between '{$dfrom}' and '{$dto}'")->fetchArray()['total_arrears'];}
                else{$arrears = $conn->query("SELECT sum(arrears) as `total_arrears` FROM `ctransaction_list` where date(date_added) between '{$dfrom}' and '{$dto}' {$user_c}")->fetchArray()['total_arrears'];}
...
<script>
    ...
    $(function(){
        ...
        $('#print').click(function(){
            ...
            el.append("<div class='text-center lh-1 fw-bold'> Amarasinghe Tyre Traders Credit Sales Report<br/>As of<br/>"+date_range+"</div><hr/>")
